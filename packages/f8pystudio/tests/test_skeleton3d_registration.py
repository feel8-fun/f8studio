...
class Skeleton3DRegistrationTests(unittest.TestCase):
    def test_operator_spec_registered(self) -> None:
        ...
        target = [s for s in specs if str(s.operatorClass or "") == "f8.skeleton3d"]
        self.assertEqual(len(target), 1)
        self.assertEqual(str(target[0].rendererClass or ""), "pystudio_skeleton3d")
    ...
    def test_renderer_registered(self) -> None:
        ...
        node_cls = render_reg.get("pystudio_skeleton3d", "default_op")
        self.assertEqual(node_cls.__name__, "PyStudioSkeleton3DNode")
