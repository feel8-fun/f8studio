...
from f8pyengineqt.services.studio_describe import describe_payload_json as studio_describe_payload_json
...
def _seed_graph(view: OperatorGraphEditor) -> None:
    ...
    start = view.create_node(operator_key(ENGINE_SERVICE_CLASS, "f8/start"), pos=(-420, 0))
    const_a = view.create_node(operator_key(ENGINE_SERVICE_CLASS, "f8/constant"), pos=(-180, -120), name="const_a")
    const_b = view.create_node(operator_key(ENGINE_SERVICE_CLASS, "f8/constant"), pos=(-180, 120), name="const_b")
    add = view.create_node(operator_key(ENGINE_SERVICE_CLASS, "f8/add"), pos=(140, 0))
    logger = view.create_node(operator_key(ENGINE_SERVICE_CLASS, "f8/log"), pos=(420, 0))
...
def _seed_operator_graph(node_graph: object) -> list[GenericNode]:
    ...
    try:
        start = node_graph.create_node(operator_key(ENGINE_SERVICE_CLASS, "f8/start"), pos=[-420.0, 0.0], **create_kwargs)
        const_a = node_graph.create_node(operator_key(ENGINE_SERVICE_CLASS, "f8/constant"), pos=[-180.0, -120.0], name="const_a", **create_kwargs)
        const_b = node_graph.create_node(operator_key(ENGINE_SERVICE_CLASS, "f8/constant"), pos=[-180.0, 120.0], name="const_b", **create_kwargs)
        add = node_graph.create_node(operator_key(ENGINE_SERVICE_CLASS, "f8/add"), pos=[140.0, 0.0], **create_kwargs)
        logger = node_graph.create_node(operator_key(ENGINE_SERVICE_CLASS, "f8/log"), pos=[420.0, 0.0], **create_kwargs)
    except Exception:
        return []
...
def _main_gui() -> None:
    ...
    from f8pyengineqt.renderers.generic import GenericNode
    from f8pyengineqt.renderers.renderer_registry import RendererRegistry
...
if __name__ == "__main__":
    ...
    parser.add_argument("--describe", action="store_true", help="Print service/operator specs as JSON and exit.")
    args, _unknown = parser.parse_known_args(sys.argv[1:])

    if args.describe:
        print(json.dumps(studio_describe_payload_json(), ensure_ascii=False))
        raise SystemExit(0)
