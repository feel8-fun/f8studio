...
from ..engine.nats_naming import (
    data_subject,
    ensure_token,
...
class ServiceRuntime:
    ...
    async def pull_data(self, node_id: str, port: str) -> Any:
        ...
        if buf is None:
            return None
        edge = buf.edge
        strategy = getattr(edge, "strategy", None) or F8EdgeStrategyEnum.latest

        if buf.last_seen_ts is not None and self._is_stale(edge, buf.last_seen_ts):
            return None
        ...
        if strategy == F8EdgeStrategyEnum.average:
            ...
            v = self._interp_buffer(buf, _now_ms())
            buf.queue.clear()
        elif strategy == F8EdgeStrategyEnum.latest:
            v = self._latest_buffer(buf)
            buf.queue.clear()
        elif strategy in (F8EdgeStrategyEnum.hold, F8EdgeStrategyEnum.repeat):
            if buf.queue:
                v = self._latest_buffer(buf)
                buf.queue.clear()
            else:
