...
void LogMpvMessage(const mpv_event_log_message* log) {
  ...
  if (std::strcmp(level, "fatal") == 0) spdlog::critical("[mpv]{}", text);
  else if (std::strcmp(level, "error") == 0) spdlog::error("[mpv]{}", text);
  else if (std::strcmp(level, "warn") == 0 || std::strcmp(level, "warning") == 0) spdlog::warn("[mpv]{}", text);
  else if (std::strcmp(level, "info") == 0 || std::strcmp(level, "status") == 0) spdlog::info("[mpv]{}", text);
  else spdlog::debug("[mpv]{}", text);
}
...
bool MpvPlayer::openMedia(const std::string& source) {
  if (!mpv_ || source.empty()) return false;
...
bool MpvPlayer::initializeGl() {
  ...
  if (!mpv_) return false;
  if (!EnsureOpenGLFunctionsLoaded()) return false;
...
void MpvPlayer::shutdownGl() {
  ...
  if (videoTextureId_) glDeleteTextures(1, &videoTextureId_);
  if (videoFboId_) glDeleteFramebuffers(1, &videoFboId_);
  videoTextureId_ = 0;
  videoFboId_ = 0;
  videoTexWidth_ = 0;
  videoTexHeight_ = 0;

  if (downsampleTextureId_) glDeleteTextures(1, &downsampleTextureId_);
  if (downsampleFboId_) glDeleteFramebuffers(1, &downsampleFboId_);
...
bool MpvPlayer::renderVideoFrame() {
  ...
  if (!renderContext_ || !glInitialized_) return false;

  if (!renderUpdatePending_.exchange(false, std::memory_order_acq_rel)) return false;
  uint64_t flags = mpv_render_context_update(renderContext_);
  if ((flags & MPV_RENDER_UPDATE_FRAME) == 0) return false;
  ...
  if (!ensureVideoTargets(width, height)) return false;
...
void MpvPlayer::processEvent(const mpv_event& event) {
  switch (event.event_id) {
    case MPV_EVENT_FILE_LOADED:
      if (playingCallback_) playingCallback_(true);
      break;
...
void MpvPlayer::handlePropertyChange(const mpv_event_property& property) {
  ...
  if (std::strcmp(property.name, "time-pos") == 0 && property.format == MPV_FORMAT_DOUBLE) {
    ...
    if (timeCallback_) timeCallback_(pos, lastDurationSeconds_.load(std::memory_order_relaxed));
    ...
    if (timeCallback_) timeCallback_(lastPositionSeconds_.load(std::memory_order_relaxed), duration);
    ...
    if (playingCallback_) playingCallback_(paused == 0);
  }
}
...
bool MpvPlayer::ensureVideoTargets(unsigned width, unsigned height) {
  if (videoTextureId_ != 0 && videoTexWidth_ == width && videoTexHeight_ == height) return true;

  if (videoTextureId_ == 0) glGenTextures(1, &videoTextureId_);
  ...
  if (videoFboId_ == 0) glGenFramebuffers(1, &videoFboId_);
  glBindFramebuffer(GL_FRAMEBUFFER, videoFboId_);
...
bool MpvPlayer::ensureDownsampleTargets(unsigned width, unsigned height) {
  if (downsampleTextureId_ != 0 && downsampleWidth_ == width && downsampleHeight_ == height) return true;

  if (downsampleTextureId_ == 0) glGenTextures(1, &downsampleTextureId_);
  ...
  if (downsampleFboId_ == 0) glGenFramebuffers(1, &downsampleFboId_);
...
bool MpvPlayer::copyFrameToSharedMemory(unsigned width, unsigned height, double&) {
  ...
  if (!shouldCopyToSharedMemory(now)) return false;

  auto [target_w, target_h] = targetDimensions(width, height);
  if (target_w == 0 || target_h == 0) return false;
  if (!sink->ensureConfiguration(target_w, target_h)) return false;
  if (!ensureDownsampleTargets(target_w, target_h)) return false;
  ...
  glBlitFramebuffer(0, 0, static_cast<GLint>(width), static_cast<GLint>(height), 0, 0, static_cast<GLint>(target_w),
                    static_cast<GLint>(target_h), GL_COLOR_BUFFER_BIT, GL_LINEAR);
  ...
  if (wrote_frame) lastShmFrameTime_ = now;
  return wrote_frame;
}
...
void* MpvPlayer::GetProcAddress(void*, const char* name) { return reinterpret_cast<void*>(SDL_GL_GetProcAddress(name)); }
