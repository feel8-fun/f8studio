...
export function SkeletonOutputPreview({ nodeId, enabled = true, className }: {
  ...
  const hasScene = output?.renderer === 'three_d' && isSkeletonScene(output.payload);
  ...
  return <div className={className} ref={hostRef} data-testid={`three-preview-${nodeId}`}>
    {!enabled ? <span className="inline-video-placeholder">Node disabled</span> :
      !hasScene ? <span className="inline-video-placeholder">Waiting for skeleton</span> :
        visible && <Suspense fallback={<span className="inline-video-placeholder">Loading 3D</span>}>
          <SkeletonViewport nodeId={nodeId} compact />
        </Suspense>}
  </div>;
}
