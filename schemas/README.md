...
- `operator.schema.json`: Operator spec (operatorClass, label/description/tags, state model with access, ports, allowAdd flags) for validating runtime operator catalogs published by engine instances; prefer `fun.feel8.op.*` prefixes to distinguish operators.
