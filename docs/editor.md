...
- Node palettes show declared ports/commands; allow adding ports/commands only when the corresponding flags are true (`allowAddInPorts`/`allowAddOutPorts`, `allowAddCommands`).
- Ports: operators and services both use `inPorts`/`outPorts`; data vs state ports are rendered distinctly and cannot be cross-connected.
- Edge creation:
  ...
  - Edge metadata follows `schemas/edge.schema.json`: `kind` (`data|state`), `scope` (`intra|cross`), optional rate strategy/queue/timeout (only for cross data edges).
...
- Persistence/export:
  ...
  - At export/split time, generate half-edges and subjects (`state.<instanceId>.set`, `f8.bus.<edgeId>`) and carry strategy metadata for cross data edges.
- Validation UX:
...
## Branch/merge nodes
- Model branch (switch/gate) and merge (select/priority) explicitly. Each data path uses its own port; merges have multiple `inPorts` (no multi-link into a single port). Topology stays static; branch nodes decide per tick which `outPort` carries data; merge nodes define a fixed selection policy.
...
- Ports: generate `inPorts`/`outPorts` on node sides, top-to-bottom.
